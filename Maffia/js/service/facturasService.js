...
const FacturasService = {
    ...
    // Calcula totales asumiendo que los precios de los platos ya
    // incluyen IGV. Por ejemplo, si un plato cuesta S/ 12.00, ese
    // es el total final antes de descuento; la base imponible es
    // 12 / 1.18 y el IGV es la diferencia.
    calcularTotales(items, opciones) {
        opciones = opciones || {};
        const totalBruto = items.reduce((s, i) => s + (parseFloat(i.subtotal) || 0), 0);

        let descuento = 0;
        if (opciones.aplicarDescuento) {
            const valor = parseFloat(opciones.valorDescuento) || 0;
            descuento =
                opciones.tipoDescuento === 'porcentaje' ? (totalBruto * valor) / 100 : valor;
            descuento = Math.max(0, Math.min(descuento, totalBruto));
        }

        const total = totalBruto - descuento;
        const subtotal = total / (1 + this.IGV_TASA);
        const igv = total - subtotal;

        return { subtotal, descuento, igv, total, totalBruto };
    },
    ...
    //           metodoPago, montoRecibido, vuelto, detallePago }
    async crear(datos) {
        if (!datos.metodoPago) {
