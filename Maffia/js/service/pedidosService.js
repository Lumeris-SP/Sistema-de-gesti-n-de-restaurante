...
const PedidosService = {
    ...
    async agregarItems(pedidoId, items) {
        for (const item of items) {
            ...
            if (errorItem) {
                console.error('[PedidosService] Error agregando item:', errorItem.message);
                continue;
            }
    ...
    async cambiarEstadoPedido(pedidoId, nuevoEstado) {
        ...
        const estadoPedido = nuevoEstado === 'Listo' ? 'listo' : 'en_preparacion';
    ...
    async _sincronizarEstadoCocinaPedido(pedidoId) {
        ...
        await window.supabaseClient
            .from('pedidos')
            .update({ estado: estadoPedido, estado_cocina: estadoGeneral })
            .eq('id', pedidoId);
    },
